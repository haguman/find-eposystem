...
$domain = Read-Host -Prompt "Please enter your domain fqdn : "
$shsdircred = New-Object -TypeName "System.Management.Automation.PSCredential" -ArgumentList ($domain + "\" +$username), $(Read-Host -Prompt "Please enter your password : " -AsSecureString)
$shsdircred | %{"password correct ? " + (New-Object System.DirectoryServices.AccountManagement.PrincipalContext([System.DirectoryServices.AccountManagement.ContextType]::Domain, $($_.GetNetworkCredential().Domain ) ) ).ValidateCredentials($_.GetNetworkCredential().UserName, $_.GetNetworkCredential().Password).ToString()}
$BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($($shsdircred.Password))
...
param ($systemname,$eposerver,$epoport,$epocred)
...
$url=  "https://" + $eposerver + ":" + $epoport + "/remote/system.find?searchText=$($systemname)"
...
$result1 = find-eposystem -systemname $(read-host -Prompt "Enter SystemName") -eposerver $eposerver -epoport $epoport -epocred $epocred
